...
import type { EntityRegistryEntry } from './entity-index.model';

// The main-side entity registry (ADR 024 decision 2). Each entry pins where an
// entity type's files live (`folder` — the single enumeration home, generalizing
...
// site; `satisfies` contextually types the `identify` params (kept inferred so
// the readonly-parameter lint skips them — the extracted shapes carry the
// parser's mutable node by design) and checks each entry against its shape.
