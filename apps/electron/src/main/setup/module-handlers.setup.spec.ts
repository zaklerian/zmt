...
describe('registerModuleHandlers', () => {
  ...
  it('reads modules per file by category, projecting scalar and stat-block fields', async () => {
    ...
    ).toMatchObject({ category: 'engine' });
    expect(
      entities.find((entity) => entity.name === 'engine_small_1')?.statBlocks
        .add_stats,
    ).toEqual([{ key: 'air_range', value: '100' }]);
    expect(
      entities.find((entity) => entity.name === 'ship_gun_1')?.category,
    ).toBe('ship_light_battery');
    expect(
      entities.find((entity) => entity.name === 'modder_widget')?.category,
